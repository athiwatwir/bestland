...
class AssetOrderController extends AppController {
    ...
    public function sales() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($result) {
                if ($this->create_customer_asset($this->request->data, $result->id)) {
    ...
    private function sendemail($customer_id = null) {
        ...
        if ($customerAsset->type == 'P') {
            $title = 'ลูกค้าฝากหาอสังหาริมทรัพย์';
            $site_to_view = SITE_URL . 'customer-assets/view-sale/' . $customerAsset->id;
            if ($this->Sendemail->asset_order_purchase_success($customer,$customerAsset, $title,$site_to_view)) {
            ...
            $title = 'ลูกค้าฝากขายอสังหาริมทรัพย์';
            $site_to_view = SITE_URL . 'customer-assets/view-sale/' . $customerAsset->id;
            if ($this->Sendemail->asset_order_sales_success($customer,$customerAsset, $title,$site_to_view)) {
                //$this->Flash->success(__('ส่ง email สำเร็จ'));
                return true;
            } else {
